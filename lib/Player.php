...
class Player 
{
  ...
  public function attempt_auto_ship_placement($ship) 
  {
    ...
    } while($this->board->cells[$pivot_point]->status === 'S');
    $pivot_point_index = $this->set_pivot_point_index($pivot_point);
    $movement_array = $this->evaluator->create_movement_array($pivot_point_index, $this->board_dimension);
    $wip_array = [];
    do {
      $x = $this->set_direction($movement_array);
